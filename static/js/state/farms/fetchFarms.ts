  ...
  const data = await Promise.all(
    farmsConfig.map(async (farmConfig) => {
      ...
      if (farmConfig.isTokenOnly) {
        ...
        // Get tokenPrice
        lpTokenPrice = tokenPriceVsQuote
        ...
        // console.log(`qutoeTokenAmount: ${quoteTokenAmount}`)
        ...
        if (quoteTokenDecimals > 6) {
          lpTokenPrice = lpTotalInQuoteToken
            .times(new BigNumber(10).pow(tokenDecimals - quoteTokenDecimals))
            .div(new BigNumber(lpTokenBalanceMC))
            .times(new BigNumber(10).pow(new BigNumber(decimalTotal).div(new BigNumber(2))))
        } else {
          lpTokenPrice = lpTotalInQuoteToken
      ...
      const [info, totalAllocPoint, BallPerBlock] = await multicall(masterchefABI, [
        {
      ...
      return {
        ...
        eggPerBlock: new BigNumber(BallPerBlock).toNumber(),
      }
    }),
  )
  return data
